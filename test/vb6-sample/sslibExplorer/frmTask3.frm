...
Begin VB.Form frmTask 
   ...
   ClientTop       =   390
   ClientWidth     =   8310
   Icon            =   "frmTask3.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   5535
   ScaleWidth      =   8310
   Begin sslibExplorer.KeyValueEditor BookEditor 
      Height          =   4320
      Left            =   75
      TabIndex        =   7
      Top             =   135
...
Public Sub InitWithBookInfo(ByRef vBookInfo As CBookInfo)
    ...
    'Debug.Print N_(i) & " = " & bookInfo(i)
      If vBookInfo(i) <> vbNullString Then BookEditor.SetField N_(i), vBookInfo(i)
    Next
...
For i = CST_SSLIB_FIELDS_LBound To CST_SSLIB_FIELDS_UBound
    'Debug.Print N_(i) & " = " & bookInfo(i)
      If bookInfo(i) <> vbNullString Then BookEditor.SetField N_(i), bookInfo(i)
Next
...
Dim i As Long
For i = CST_SSLIB_FIELDS_LBound To CST_SSLIB_FIELDS_UBound
    If vData(i) <> vbNullString Then BookEditor.SetField N_(i), vData(i)
Next
'
'With vData
'    If .Author <> vbNULLSTRING Then SetField N_(SSF_Author), .Author
'    If .title <> vbNULLSTRING Then SetField N_(SSF_Title), .title
'    If .Publisher <> vbNULLSTRING Then SetField N_(SSF_Publisher), .Publisher
'    If .SSID <> vbNULLSTRING Then SetField N_(SSF_SSID), .SSID
'    If .Header <> vbNULLSTRING Then SetField N_(SSF_HEADER), .Header
'    If .URL <> vbNULLSTRING Then SetField N_(SSF_URL), .URL
'    If .About <> vbNULLSTRING Then SetField N_(SSF_Comments), .About
'    If .Subject <> vbNULLSTRING Then SetField N_(SSF_Subject), .Subject
'    If .PagesCount <> vbNULLSTRING Then SetField N_(SSF_PagesCount), .PagesCount
'    If .PublishedDate <> vbNULLSTRING Then SetField N_(SSF_PublishDate), .PublishedDate
'End With
End Sub
...
Public Sub Reset()
    ...
    For i = CST_SSLIB_FIELDS_LBound To CST_SSLIB_FIELDS_UBound
        If i <> SSLIBFields.SSF_SAVEDIN Then
            BookEditor.SetField N_(i), vbNullString
        End If
    Next
End Sub
...
'
    ...
    'Timer.Interval = 0
    If Timer Is Nothing Then Set Timer = New CTimer
    If chkDetectClipboard.Value Then
...
Private Sub Form_Load()
    ...
    BookEditor.AddItem "任务名", VCT_NORMAL, vbNullString, False
    
    Dim i As Long
    For i = CST_SSLIB_FIELDS_LBound To CST_SSLIB_FIELDS_TASKS_UBOUND
        BookEditor.AddItem SSLIB_ChnFieldName(i), VCT_NORMAL, vbNullString, False
...
Private Sub DetectClipboard()
    '<EhHeader>
    On Error GoTo DetectClipboard_Err
    '</EhHeader>
    Dim vText As String
    vText = Clipboard.GetText
    If vText <> vbNullString Then
        'Clipboard.Clear
...
DetectClipboard_Err:
    Debug.Print "sslibExplorer.frmTask.DetectClipboard:Error " & Err.Description
    Err.Clear
...
Private Function CheckValue() As Boolean
    CheckValue = True
    If BookEditor.GetField(SSLIB_ChnFieldName(SSF_Title)) = vbNullString And BookEditor.GetField(SSLIB_ChnFieldName(SSF_SSID)) = vbNullString Then
        MsgBox QuoteString(SSLIB_ChnFieldName(SSF_Title)) & "和" & QuoteString(SSLIB_ChnFieldName(SSF_Title)) & "不能都为空", vbCritical
        CheckValue = False
        'Exit Function
    End If

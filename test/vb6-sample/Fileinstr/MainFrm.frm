...
FiletreeSearch txtDIRSearch.Text, txtCT.Text, Val(txtTimesMin.Text)

End Sub

Public Sub FiletreeSearch(thedir As String, thetext As String, MTimes As Integer)
...
a = FileInStrTimes(fsofile.Path, thetext, MTimes)
If a > 0 Then
...
FiletreeSearch fsofolder.Path, thetext, MTimes
Next
...
fso.DeleteFile lvResult.ListItems(i)
...
If lvResult.ListItems(i).Selected = True Then
On Error Resume Next
fso.DeleteFile fso.BuildPath(dstdir, fso.GetFileName(lvResult.ListItems(i)))
fso.MoveFile lvResult.ListItems(i), fso.BuildPath(dstdir, fso.GetFileName(lvResult.ListItems(i)))
lvResult.ListItems.Remove i
End If
Next
End Sub

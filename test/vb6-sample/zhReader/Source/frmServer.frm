...
Private Sub Sck_DataArrival(Index As Integer, ByVal bytesTotal As Long)
    ...
    If rData Like "GET * HTTP/1.?*" Then
        ...
        If zhUrlNew.sZipName = "" Or zhUrlNew.sHtmlPath = "" Then
            sFileWanted = BuildPath(App.Path, cHtmlAboutFilename)
        ElseIf isFakeOne(zhUrlNew.sHtmlPath, sRealUrl) Then
            sFileWanted = BuildPath(sTempZip, sRealUrl)
        Else
            sFileWanted = BuildPath(sTempZH, zhUrlNew.sHtmlPath)
           '解压文件
           If PathExists(sFileWanted) = False Then
                MainFrm.myXUnzip zhUrlNew.sZipName, _
                                              ...
                                              True
            End If
            If PathType(sFileWanted) = LNFolder Then sFileWanted = ""
        End If
'
...
'        End If
'
        If sFileWanted <> "" And PathExists(sFileWanted) = True Then
        
            Select Case LCase$(GetExtensionName(sFileWanted))
            Case "txt", "text"

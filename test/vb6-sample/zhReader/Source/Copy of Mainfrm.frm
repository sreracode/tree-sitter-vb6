...
'    Next
    ...
    If listIndex = lwFiles Then
        For i = 1 To lzipCount
            zipContent(1, i) = sFilesInZip.Item(i)
            zipContent(2, i) = zipContent(1, i)
        Next
    Else
        For i = 1 To lzipCount
            zipContent(1, i) = sFilesINContent(1).Item(i)
            zipContent(2, i) = sFilesINContent(2).Item(i)
        Next
    End If
    'Set sFoldersInZip = Nothing
    '    zhrStatus.iListIndex = lwFiles
...
Public Sub mnufile_Open_Click()
    ...
    fResult = cDLG.VBGetOpenFileName( _
       FileName:=thisfile, _
       Filter:="Zippacked Html File|*.zhtm;*.zip;*.zbook|所有文件|*.*", _
       InitDir:=sInitDir, _

...
Begin VB.Form MainFrm 
   ...
   Begin VB.Frame theShow 
      ...
      Begin VB.ComboBox cmbAddress 
         ...
         EndProperty
         Height          =   312
         ItemData        =   "Mainfrm.frx":2C32
         Left            =   240
         List            =   "Mainfrm.frx":2C34
         TabIndex        =   8
         Top             =   0
         Width           =   3492
      End
...
Private Sub List_Collapse(Index As Integer, ByVal Node As MSComctlLib.Node)

    Node.Image = 1

End Sub

Private Sub List_Expand(Index As Integer, ByVal Node As MSComctlLib.Node)

    Node.Image = 2

End Sub

Private Sub list_NodeClick(Index As Integer, ByVal Node As MSComctlLib.Node)
...
Private Sub mnuDir_random_Click()
    ...
    sCur = linvblib.gCFileSystem.LookFor(sPath, LN_FILE_RAND, "*.zhtm")
...
Private Sub mnuDir_readNext_Click()
    ...
    sPath = zhrStatus.sCur_zhFile
    If sPath = "" Then sPath = CurDir$
    sCur = linvblib.gCFileSystem.LookFor(sPath, LN_FILE_next, "*.zhtm")
    If sCur <> "" Then MainFrm.loadzh sCur
End Sub
...
Private Sub mnuDir_readPrev_Click()
    ...
    sPath = zhrStatus.sCur_zhFile
    If sPath = "" Then sPath = CurDir$
    sCur = linvblib.gCFileSystem.LookFor(sPath, LN_FILE_prev, "*.zhtm")
    If sCur <> "" Then MainFrm.loadzh sCur
...
Public Sub mnufile_Open_Click()
    ...
    fResult = cDLG.VBGetOpenFileName( _
       filename:=thisfile, _
       Filter:="Zippacked Html File|*.zhtm;*.zip|所有文件|*.*", _
       InitDir:=sInitDir, _
...
Sub zhReaderReset()
    ...
    's_AI_DefaultFile = ""
    Set sFilesInZip = New CStringVentor   'CStringCollection

...
Dim iPos As Long
   ...
   'MsgBox "MessageCallBack Begin"
     ...
     sFilename = mShareFunction.CBytesToStr(fname.ch)
     sMethod = mShareFunction.CBytesToStr(meth.ch)
...
Dim sMsg As String
   ...
   If x > 1 And x < 32000 Then
    ...
    sMsg = mShareFunction.CBytesToStr(fname.ch)
...
On Error Resume Next
   ...
   ' Ask for password:
   sName = mShareFunction.CBytesToStr(name.ch)
   ...
   mShareFunction.StrToCBytes sPassword, pwd.ch
...
Dim sFile As String
   ...
   sFile = mShareFunction.CBytesToStr(fname.ch)
...
On Error Resume Next
   ...
   If x > 1 And x < 32000 Then
      ...
      sInfo = mShareFunction.CBytesToStr(mname.ch)

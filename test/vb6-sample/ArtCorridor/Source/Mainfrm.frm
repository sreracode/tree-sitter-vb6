...
Private Const MAX_IMAGEBOX As Integer = 10
Enum FreeImageDirection
    FID_UP = -1
    FID_DOWN = 1
End Enum
...
Public Function randomView(Optional bRestart As Boolean = False) As Boolean
    ...
    'Else
        GetView sFilesInZip.value(iRandomArr(iViewNow)), ImageBox(0)
   ' End If
...
Private Sub Form_Resize()
    ...
    With ImageBox(0) '(i - 1)
        ...
        If .Width <= theShow.Width Then
            .Left = (theShow.Width - .Width - VBOXScroll.Width) / 2
        ...
        Else
            ...
            .Left = 0
            HBoxScroll.Min = 0
            HBoxScroll.Max = ImageBox(0).Width - theShow.Width
        End If
        
        If .Height <= theShow.Height Then
            If HBoxScroll.Visible Then
                .Top = (theShow.Height - .Height - HBoxScroll.Height) / 2
            Else
        ...
        Else
            ...
            VBOXScroll.Min = 0
            VBOXScroll.Max = ImageBox(0).Height - theShow.Height
        End If
...
Public Sub GetView(ByVal shortfile As String, ByRef inBox As Image)
    ...
    W = theShow.Width '- HBoxScroll.Height
    H = theShow.Height '- VBOXScroll.Width
    ...
    End Select
        ...
        inBox.Width = imgWidth * R
        inBox.Height = imgHeight * R
    
    NotResize = False
    

    Set inBox.Picture = imgToLoad
...
ImageBox(0).Move -HBoxScroll.value, ImageBox(0).Top
End Sub




Private Sub ImageBox_MouseDown(Index As Integer, Button As Integer, Shift As Integer, x As Single, y As Single)
    If Button = 1 Then
        mnuGo_Next_Click
...
Private Sub list_NodeClick(ByVal Node As MSComctlLib.Node)
    ...
    Dim sTag As String
    sTag = Node.Tag
    If Right$(sTag, 1) <> "/" Then GetView sTag, ImageBox(0)
...
Public Sub loadzh(ByVal thisfile As String, Optional ByVal firstfile As String = "", Optional Reloadit As Boolean = False)
    ...
    With zhrStatus
        ...
        If Reloadit = False And thisfile = .sCur_zhFile Then
            ...
            If firstfile <> "" Then
                GetView firstfile, ImageBox(0)
                Exit Sub
            ElseIf .sCur_zhSubFile <> "" Then
                GetView .sCur_zhSubFile, ImageBox(0)
                Exit Sub
            End If
...
'    End With
    ...
    Else
        ...
        If nowAt.page <> "" Then
            ...
            bScroll = True
            GetView nowAt.page, ImageBox(0)
        Else
            GetView sFilesInZip.value(1), ImageBox(0)
        End If
    End If
...
Private Sub mnu_mode_fit_height_Click()
    ...
    GetView zhrStatus.sCur_zhSubFile, ImageBox(0)
End Sub
...
Public Sub mnuGo_Home_Click()
    ...
    If zhrStatus.sCur_zhFile = "" Or sFilesInZip.Count < 1 Then
        appAbout
    Else
        GetView sFilesInZip(1), ImageBox(0)
    End If
...
Public Sub mnuGo_Next_Click()
    ...
    Dim lcurPage As Long
        ...
        GetView sFilesInZip.value(lcurPage + 1), ImageBox(0)
...
Private Sub VBOXScroll_Change()
    ImageBox(0).Move ImageBox(0).Left, -VBOXScroll.value
End Sub
...
Private Function getFreeImageBox(ByVal UORD As FreeImageDirection) As Image
    Static iBoxUsed As Integer
    Static ifreeBox As Integer
    
    ifreeBox = ifreeBox + UORD
    If ifreeBox < 1 Then ifreeBox = iBoxUsed
    If ifreeBox > iBoxUsed Then ifreeBox = 1
    
    
    If ImageBox(ifreeBox) Is Nothing Then
        Load ImageBox(ifreeBox)
        ImageBox(ifreeBox).Parent = theShow
        Set getFreeImageBox = ifreeBox
    End If
    
End Function

Private Function AddImagebox(ByRef imgBox As Image, ByRef frmBox As Frame, ByRef HORV As ImageDisplayMode) As Image
    Set imgBox.Parent = frmBox
